...
class Sine:
    ...
    def isEqual(self, other):
        if not isinstance(other, Sine):
            return False
        return child.isEqual(other.child)
...
class Asine:
    ...
    def eval(self):
        return math.asin(self.child.eval())

    def derivative(self, var):
        return Mult(Div(Const(1), Pow(Sub(Const(1), Pow(self.child, 2)), 0.5)), self.child.derivative(var))

    def __str__(self):
        return f"asin({self.child}"
    
    def isEqual(self, other):
        if not isinstance(other, Asine):
            return False
