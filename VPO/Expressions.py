...
def gradDescent(expr, params, xs, ys, x):
    stepsize = 100
    for i in range(50):
        gradient = grad(expr, params, xs, ys, x)
        print(gradient)
        print(stepsize)
        for j in range(len(params)):
            print(f"params[{j}] = {params[j].value}")
            params[j].value -= stepsize * gradient[j]
            stepsize *= 0.9

def grad(expr, params, xs, ys, x):
    result = []
    for p in params:
        result.append(gradComp(expr, params, xs, ys, x, p))
    return result

def gradComp(expr, params, xs, ys, x, p):
    total = 0
    for i in range(len(xs)):
        x.value = xs[i]
        value = expr.eval()
        der = simplify(expr.derivative(p))
        print(f"difference = {value - ys[i]}")
        print(f"derivative = {der.eval()}")
        total += 2 * (value - ys[i]) * der.eval()
    return total
